...
from pipelines.ppocr_v4 import PPOCRv4
from pipelines.utils.operators import np2base64


class DocParser(object):
    """DocParser"""

    def __init__(self, ocr_config_path, ocr_lang="ch", layout_analysis=False):
        self.ocr_lang = ocr_lang
        self.layout_analysis = layout_analysis
        self.ocr_infer_model = PPOCRv4(ocr_config_path)
    ...
    def ocr(self, image, det=True, rec=True, cls=None):
        ...
        if not self.layout_analysis:
            ocr_result = self.ocr_infer_model.predict(image)
            for segment in ocr_result:
                box = segment[0]
                box = _get_box(box)
